...
/*** [ phth: vergleiche folgende query mit nächster query => natural join gibt KEINEN ambiguity error für sID, die nächste query mit cross-product "," schon ! ] ***/
select distinct sID
from Student natural join Apply;

/*** [query above] Would get ambiguity error with cross-product ***/
...
/*** Rewrite using JOIN and USING ***/
/*** (error: most SQL systems allow either "ON" or "USING" in ***/
/*** combination with a "JOIN", but not both together) ***/
...
select sName, sID, cName, major
from Apply natural left outer join Student; --asymmetric operator!
