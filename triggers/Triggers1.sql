...
/**************************************************************
  ...
  phth: if we use "after" instead of "before", we would
    have to check if we created duplicate college names
**************************************************************/
...
begin
  select raise(ignore); -- "raise" does NOT raise an error (ie. the SQLite log will not show any error), it just stops the update that is underway without logging any message
end;
...
create trigger R7
after insert on Student -- after !
for each row
when New.sizeHS < 100 or New.sizeHS > 5000
begin
  delete from Student where sID = New.sID; -- must delete manually because tuples have been already inserted
end;
